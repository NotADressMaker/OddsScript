...
def run_file(filename: str, *, mode: str = "safe", max_steps: int | None = None, max_loop: int | None = None, max_recursion: int | None = None, no_io: bool = True, allow_read_dirs: list[str] | None = None, allow_domains: list[str] | None = None, audit_log: str | None = None):
    """Execute a SportsBetLang file"""
    try:
        with open(filename, 'r') as f:
            source = f.read()

        run(source, filename, mode=mode, max_steps=max_steps, max_loop=max_loop, max_recursion=max_recursion, no_io=no_io, allow_read_dirs=allow_read_dirs, allow_domains=allow_domains, audit_log=audit_log)
...
def run(source: str, filename: str = "<stdin>", *, mode: str = "safe", max_steps: int | None = None, max_loop: int | None = None, max_recursion: int | None = None, no_io: bool = True, allow_read_dirs: list[str] | None = None, allow_domains: list[str] | None = None, audit_log: str | None = None):
    ...
    try:
        ...
        interpreter = Interpreter(limits=limits, source=source, capabilities=HostCapabilities(no_io=no_io, allow_read_dirs=allow_read_dirs or [], allow_domains=allow_domains or []))
...
def repl(*, mode: str = "safe", max_steps: int | None = None, max_loop: int | None = None, max_recursion: int | None = None, no_io: bool = True, allow_read_dirs: list[str] | None = None, allow_domains: list[str] | None = None, audit_log: str | None = None):
    ...
    interpreter = Interpreter(limits=limits, source="", capabilities=HostCapabilities(no_io=no_io, allow_read_dirs=allow_read_dirs or [], allow_domains=allow_domains or []))
...
def main() -> int:
    ...
    try:
        if args.format or args.lint:
            ...
            return 0
        if args.source:
            run_file(args.source, mode=args.mode, max_steps=args.max_steps, max_loop=args.max_loop, max_recursion=args.max_recursion, no_io=args.no_io, allow_read_dirs=args.allow_read_dir, allow_domains=args.allow_domain, audit_log=args.audit_log)
        else:
            repl(mode=args.mode, max_steps=args.max_steps, max_loop=args.max_loop, max_recursion=args.max_recursion, no_io=args.no_io, allow_read_dirs=args.allow_read_dir, allow_domains=args.allow_domain, audit_log=args.audit_log)
