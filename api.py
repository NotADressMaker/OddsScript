...
class LiveStreamManager:
    ...
    def _prune_locked(self) -> None:
        if not self.subscribers:
            return
        cutoff = datetime.utcnow().timestamp() - self.max_idle_seconds
        self.subscribers = [
            subscriber
            for subscriber in self.subscribers
            if subscriber.last_seen.timestamp() >= cutoff
        ]
    ...
    async def stats(self) -> dict:
        async with self._lock:
            return {
                ...
                "dropped_messages": sum(sub.dropped_messages for sub in self.subscribers)
            }
