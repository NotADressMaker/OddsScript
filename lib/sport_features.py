...
class NBAFeatures:
    ...
    @staticmethod
    def create_recent_ats_trend(ats_margins: List[float],
                                decay: float = 0.85) -> float:
        """
        Calculate recent ATS trend using recency-weighted margins vs spread.

        Args:
            ats_margins: List of ATS margins (team margin - spread),
                ordered from most recent to oldest.
            decay: Recency decay factor applied to older games.

        Returns:
            Weighted average ATS margin; 0.0 if no history is provided.
        """
        if not ats_margins:
            return 0.0
