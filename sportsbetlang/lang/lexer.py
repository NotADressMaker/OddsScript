...
class Lexer:
    ...
    def __init__(self, source: str, limits: RuntimeLimits = DEFAULT_LIMITS) -> None:
        self.source = source
        self.limits = limits
        self.pos = 0
        self.line = 1
    ...
    def tokenize(self) -> List[Token]:
        while self.current_char():
            ...
            if self.current_char().isdigit():
                self._emit(self.read_number())
                continue

            # Strings
            if self.current_char() in '"\'':
                self._emit(self.read_string())
                continue

            # Identifiers and keywords
            if self.current_char().isalpha() or self.current_char() == '_':
                self._emit(self.read_identifier())
                continue
            ...
            if current == '=' and next_char == '=':
                self._emit(Token(TokenType.EQUAL, None, start_line, start_col))
                self.advance()
                self.advance()
                continue

            if current == '!' and next_char == '=':
                self._emit(Token(TokenType.NOT_EQUAL, None, start_line, start_col))
                self.advance()
                self.advance()
                continue

            if current == '<' and next_char == '=':
                self._emit(Token(TokenType.LESS_EQUAL, None, start_line, start_col))
                self.advance()
                self.advance()
                continue

            if current == '>' and next_char == '=':
                self._emit(Token(TokenType.GREATER_EQUAL, None, start_line, start_col))
                self.advance()
                self.advance()
                continue

            if current == '-' and next_char == '>':
                self._emit(Token(TokenType.ARROW, None, start_line, start_col))
                self.advance()
                self.advance()
                continue
            ...
            if current in single_char_tokens:
                self._emit(Token(single_char_tokens[current], None, start_line, start_col))
                self.advance()
                continue
        ...
        self._emit(Token(TokenType.EOF, None, self.line, self.column))
        return self.tokens
